...
#frame-based timing
#============================
refresh=1.0/60.0                                                                #refresh frquency of computer = 60Hz
#set durations
fix_dur = 0.5               #500 ms
image_dur = 1.0             #1000 ms
text_dur = 0.5              #500 ms

#set frame counts
fix_frames = int(fix_dur / refresh)                                             #whole number (using int() function)
...
core.wait(2.0)                                                                  #wait for keypress
...
for block in nBlocks:
  ...
  #  counter = counter + 1
    ...
    for trial in nTrials:
        ...
        #START TRIAL
        #=====================   
        for frameN in range(total_frames):                                      #frame timing
        ...
        # stimTimer.reset()
        # while stimTimer.getTime() <= .5:
                textStim.text = trial_msg                                       #draw
                textStim.draw() 
                win.flip()                                                      #show
        # trialEnd = trialTimer.getTime()
        print('Overall, %i frame(s) was (were) dropped.' % win.nDroppedFrames)  #add check output for frames dropped    
